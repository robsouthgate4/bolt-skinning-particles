import { Bolt, Program, FBO, Mesh, DrawSet, Texture2D } from "bolt-gl";
...
export default class FBOSim {
  ...
  bindFBOs(fboDefinitions: FBOSwapDefinition[]) {
    ...
    this._FBOSwapDefinitions.forEach((fboSwapDefinition) => {
      ...
      if (fboSwapDefinition.program) {
        fboSwapDefinition.program.activate();

        fboSwapDefinition.program.setVector2(
          "resolution",
          vec2.fromValues(
