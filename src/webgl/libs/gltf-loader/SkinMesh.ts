...
import { Mesh, Node, Program, GeometryBuffers, MeshParams, Texture2D, RGBA, CLAMP_TO_EDGE, NEAREST, FLOAT, RGBA32f, RGB, Bolt, flattenFloatArray } from "bolt-gl";
import Skin from "./Skin";

export default class SkinMesh extends Mesh {
  private _skin: Skin | undefined;
  private _jointTexture: Texture2D | undefined;
  private _jointTextureBound = false;
  ...
  draw(program: Program, node: Node) {
    ...
    if (!this._jointTextureBound) {
      program.setTexture("jointTexture", this._jointTexture);
      this._jointTextureBound = true;
    }
